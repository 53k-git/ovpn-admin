...
func (oAdmin *OvpnAdmin) userEnableTOTPHandler(w http.ResponseWriter, r *http.Request) {
	log.Info(r.RemoteAddr, " ", r.RequestURI)
	_ = r.ParseForm()
	if !*authByTOTP {
	...
	if err != nil {
		log.Error(err)
		http.Error(w, `{"status":"error", "message":"Failed to generate TOTP secret"}`, http.StatusInternalServerError)
		return
	}
	...
	if err != nil {
		log.Error(err)
		http.Error(w, `{"status":"error", "message":"Failed to generate QR code"}`, http.StatusInternalServerError)
		return
	}

	w.WriteHeader(http.StatusOK)
	fmt.Fprintf(w, `{"status":"ok", "secret":"%s", "qrCode":"data:image/png;base64,%s"}`, key.Secret(), qrCode)
}

func (oAdmin *OvpnAdmin) userVerifyTOTPHandler(w http.ResponseWriter, r *http.Request) {
	log.Info(r.RemoteAddr, " ", r.RequestURI)
	_ = r.ParseForm()
	if !*authByTOTP {
		http.Error(w, `{"status":"error", "message":"TOTP not enabled"}`, http.StatusNotImplemented)
	...
	if username == "" || code == "" {
		http.Error(w, `{"status":"error", "message":"Username and code required"}`, http.StatusBadRequest)
		return
	}
	...
	if totpSecret == nil {
		http.Error(w, `{"status":"error", "message":"TOTP not configured for user"}`, http.StatusNotFound)
		return
	}

	// Verify the code
	if !verifyTOTP(totpSecret.Secret, code) {
		http.Error(w, `{"status":"error", "message":"Invalid TOTP code"}`, http.StatusUnauthorized)
		return
	}
	...
	if !totpSecret.Enabled {
		...
		log.Infof("TOTP enabled for user %s", username)
	}

	w.WriteHeader(http.StatusOK)
	fmt.Fprintf(w, `{"status":"ok", "message":"TOTP verification successful"}`)
}

func (oAdmin *OvpnAdmin) userDisableTOTPHandler(w http.ResponseWriter, r *http.Request) {
	log.Info(r.RemoteAddr, " ", r.RequestURI)
	_ = r.ParseForm()
	if !*authByTOTP {
		http.Error(w, `{"status":"error", "message":"TOTP not enabled"}`, http.StatusNotImplemented)
	...
	log.Infof("TOTP disabled for user %s", username)
	w.WriteHeader(http.StatusOK)
	fmt.Fprintf(w, `{"status":"ok", "message":"TOTP disabled successfully"}`)
}

func (oAdmin *OvpnAdmin) userGetTOTPStatusHandler(w http.ResponseWriter, r *http.Request) {
	log.Info(r.RemoteAddr, " ", r.RequestURI)
	_ = r.ParseForm()
	if !*authByTOTP {
		http.Error(w, `{"status":"error", "message":"TOTP not enabled"}`, http.StatusNotImplemented)
	...
	if totpSecret == nil {
		w.WriteHeader(http.StatusOK)
		fmt.Fprintf(w, `{"status":"ok", "enabled":false}`)
		return
	}
...
func main() {
	...
	if *authByTOTP {
		if *storageBackend != "kubernetes.secrets" {
			...
			log.Fatal("Right now the keys `--storage.backend=kubernetes.secret` and `--auth.totp` are not working together. Please use only one of them ")
		}
	}
	...
	http.HandleFunc(*listenBaseUrl+"api/user/totp/enable", ovpnAdmin.userEnableTOTPHandler)
